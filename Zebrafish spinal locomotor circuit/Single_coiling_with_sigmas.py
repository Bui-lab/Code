...
def connectome_coiling(rand=0, stim0=8, sigma=0, sigmaD=0, sigmaL = 0, sigmaP=0, sigmaW=0,   
    ...
    tmax += 200 # add 200 ms to give time for initial conditions to settle.  This initial 200 ms is not used in the data analysis
    
    nmax = int(tmax/dt)
    
    ## Declare Neuron Types
    
    L_IC = [Izhikevich_9P(a=0.0005*gauss(1,sigmaP),b=0.5*gauss(1,sigmaP),c=-30*gauss(1,sigmaP), d=5*gauss(1,sigmaP), vmax=0*gauss(1,sigmaP), vr=-60*gauss(1,sigmaP), vt=-45*gauss(1,sigmaP), k=0.05*gauss(1,sigmaP), Cm = 50*gauss(1,sigmaP), dt=dt, x=1.0*gauss(1, sigma),y=-1) for i in range(nIC)]
    R_IC = [Izhikevich_9P(a=0.0005*gauss(1,sigmaP),b=0.5*gauss(1,sigmaP),c=-30*gauss(1,sigmaP), d=5*gauss(1,sigmaP), vmax=0*gauss(1,sigmaP), vr=-60*gauss(1,sigmaP), vt=-45*gauss(1,sigmaP), k=0.05*gauss(1,sigmaP), Cm = 50*gauss(1,sigmaP), dt=dt, x=1.0*gauss(1, sigma),y=1) for i in range(nIC)]
    
    L_MN = [ Izhikevich_9P(a=0.5*gauss(1,sigmaP),b=0.1*gauss(1,sigmaP),c=-50*gauss(1,sigmaP), d=0.2*gauss(1,sigmaP), vmax=10*gauss(1,sigmaP), vr=-60*gauss(1,sigmaP), vt=-45*gauss(1,sigmaP), k=0.05*gauss(1,sigmaP), Cm = 20*gauss(1,sigmaP), dt=dt, x=5.0+1.6*i*gauss(1, sigma),y=-1) for i in range(nMN)]
    R_MN = [ Izhikevich_9P(a=0.5*gauss(1,sigmaP),b=0.1*gauss(1,sigmaP),c=-50*gauss(1,sigmaP), d=0.2*gauss(1,sigmaP), vmax=10*gauss(1,sigmaP), vr=-60*gauss(1,sigmaP), vt=-45*gauss(1,sigmaP), k=0.05*gauss(1,sigmaP), Cm = 20*gauss(1,sigmaP), dt=dt, x=5.0+1.6*i*gauss(1, sigma),y=1) for i in range(nMN)]
    
    L_V0d = [ Izhikevich_9P(a=0.5*gauss(1,sigmaP),b=0.01*gauss(1,sigmaP),c=-50*gauss(1,sigmaP), d=0.2*gauss(1,sigmaP), vmax=10*gauss(1,sigmaP), vr=-60*gauss(1,sigmaP), vt=-45*gauss(1,sigmaP), k=0.05*gauss(1,sigmaP), Cm = 20*gauss(1,sigmaP), dt=dt, x=5.0+1.6*i*gauss(1, sigma),y=-1) for i in range(nV0d)]
    R_V0d = [ Izhikevich_9P(a=0.5*gauss(1,sigmaP),b=0.01*gauss(1,sigmaP),c=-50*gauss(1,sigmaP), d=0.2*gauss(1,sigmaP), vmax=10*gauss(1,sigmaP), vr=-60*gauss(1,sigmaP), vt=-45*gauss(1,sigmaP), k=0.05*gauss(1,sigmaP), Cm = 20*gauss(1,sigmaP), dt=dt, x=5.0+1.6*i*gauss(1, sigma),y=1) for i in range(nV0d)]
    ...
    #Lists storing residuals for solving membrane potentials
    resLIC=zeros((nIC,3))
    resRIC=zeros((nIC,3))
    resLMN=zeros((nMN,3))
    resRMN=zeros((nMN,3))
    ...
    for k in range (0, nIC):
        for l in range (0, nIC):
            if (k!= l):                                         #no distance conditions because it is a kernel
    ...
    for k in range (0, nIC):
        for l in range (0, nMN):
            if (0.2<LD_IC_MN[k,l]<10*gauss(1, sigmaL) and L_IC[k].x < L_MN[l].x): #second condition is because this connection is descending
    ...
    for k in range (0, nIC):
        for l in range (0, nV0d):
            if (0.2<LD_IC_V0d[k,l]<10*gauss(1, sigmaL) and L_IC[k].x < L_V0d[l].x):   #second condition is because this connection is descending
    ...
    for k in range (0, nMN):
        for l in range (0, nMuscle):
            if (L_Muscle[l].x-1<L_MN[k].x< L_Muscle[l].x+1):         #segmental connection, no sigmaL
    ...
    for k in range (0, nIC):
        for l in range (0, nMN):
            if (0.2<RD_IC_MN[k,l]<10*gauss(1, sigmaL) and R_IC[k].x < R_MN[l].x):   #second condition is because this connection is descending
    ...
    for k in range (0, nIC):
        for l in range (0, nV0d):
            if (0.2<RD_IC_V0d[k,l]<10*gauss(1, sigmaL) and R_IC[k].x < R_V0d[l].x):   #second condition is because this connection is descending
    ...
    ## Main loop for calculating membrane potential at all time points
    ...
    for t in range (0, nmax):
        ...
        ## Calculate synaptic currents between pairs of populations
        for k in range (0, nV0d):
            for l in range (0, nMN):
        ...
        for k in range (0, nIC):
            ...
            resLIC[k,:] = L_IC[k].getNextVal(resLIC[k,0],resLIC[k,1], stim[t]*gauss(1.0, sigmaD) + sum(LSGap_IC_IC[:,k]) - sum(LSGap_IC_IC[k,:]) + sum(LSGap_MN_IC[:,k]) - sum(LSGap_IC_MN[k,:]) + sum(LSGap_V0d_IC[:,k]) - sum(LSGap_IC_V0d[k,:]) + IsynL)
            VLIC[k,t] = resLIC[k,0]
        
            if t < 32000: # This is to delay the stim to right ICs so that they don't fire synchronously
                right_ON = 0
            else:
                right_ON = 1
            resRIC[k,:] = R_IC[k].getNextVal(resRIC[k,0],resRIC[k,1], stim[t]*right_ON*0 + sum(RSGap_IC_IC[:,k])  - sum(RSGap_IC_IC[k,:])  + sum(RSGap_MN_IC[:,k]) - sum(RSGap_IC_MN[k,:]) + sum(RSGap_V0d_IC[:,k]) - sum(RSGap_IC_V0d[k,:]) + IsynR)
            VRIC[k,t] = resRIC[k,0]
      ...
      ## Removing the first 200 ms to let settle the initial conditions.
