...
class Single_coil_IC_KO(Single_coil_base):
    ...
    def calcICPotentialsandResidues(self, t):
        ...
        for k in range (0, self.nIC):
            ...
            if t > self.tICKOstart and t < self.tICKOend:
                self.resLIC[k,:] = self.L_IC[k].getNextVal(self.resLIC[k,0],self.resLIC[k,1], 0)
                self.resRIC[k,:] = self.R_IC[k].getNextVal(self.resRIC[k,0],self.resRIC[k,1], 0)
            else:
                ...
                self.resRIC[k,:] = self.R_IC[k].getNextVal(self.resRIC[k,0],self.resRIC[k,1], IgapR + IsynR)
