...
def connectome_coiling(rand=0, stim0=8, sigma=0,
    ...
    for t in range (0, nmax):
        ...
        for k in range (0, nIC):
            ...
            resRIC[k,:] = R_IC[k].getNextVal(resRIC[k,0],resRIC[k,1], sum(RSGap_IC_IC[:,k])  - sum(RSGap_IC_IC[k,:])  + sum(RSGap_MN_IC[:,k]) - sum(RSGap_IC_MN[k,:]) + sum(RSGap_V0d_IC[:,k]) - sum(RSGap_IC_V0d[k,:]) + IsynR)
        ...
        for k in range (0, nV0d):
            if t > 50000 and t < 100000:
                resLV0d[k,:] = L_V0d[k].getNextVal(resLV0d[k,0],resLV0d[k,1], 0)
                resRV0d[k,:] = R_V0d[k].getNextVal(resRV0d[k,0],resRV0d[k,1], 0)
            else:
