...
def connectome_double_coiling(rand=0, stim0=8, sigma=0,
    ...
    for t in range (0, nmax):
        ...
        for k in range (0, nV0v):        
            resLV0v[k,:] = L_V0v[k].getNextVal(resLV0v[k,0],resLV0v[k,1], - sum(LSGap_V0v_IC[k,:]) + sum(LSGap_IC_V0v[:,k]) - sum(LSGap_V0v_V0v[k,:]) + sum(LSGap_V0v_V0v[:,k]) -sum(LSGap_V0v_MN[k,:])  + sum(LSGap_MN_V0v[:,k]))
            VLV0v[k,t] = resLV0v[k,0]
            resRV0v[k,:] = R_V0v[k].getNextVal(resRV0v[k,0],resRV0v[k,1], - sum(RSGap_V0v_IC[k,:]) + sum(RSGap_IC_V0v[:,k]) - sum(RSGap_V0v_V0v[k,:]) + sum(RSGap_V0v_V0v[:,k]) -sum(RSGap_V0v_MN[k,:])  + sum(RSGap_MN_V0v[:,k]))
